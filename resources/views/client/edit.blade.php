<div class="card bg-none card-box">
  {{Form::model($customer,array('route' => array('client.update', $customer->id), 'method' => 'PUT')) }}
        ...
        <div class="form-icon-user">
          <select name="organization_type" class="form-control custom-select">
            ...
            @foreach($organisations as $organisation)
              <option value="{{$organisation->id}}" {{($organisation->id == $customer->organization_type) ? 'selected' : ''}}>{{$organisation->name}}</option>
            @endforeach
          </select>
        </div>
      </div>
        ...
        <div class="form-icon-user">
          <select name="atc_list_id" class="form-control custom-select">
            ...
            @foreach($fui_atc_lists as $atc_list)
              <option value="{{$atc_list->id}}" {{($atc_list->id == $customer->atc_list_id) ? 'selected' : ''}}>
      ...
      <div class="form-group">
        ...
        <div class="form-icon-user">
          <select name="tax_type" class="form-control custom-select">
            ...
            <option value="VATABLE" {{($customer->tax_type == 'VATABLE') ? 'selected' : ''}}>VATABLE</option>
            <option value="NON VATABLE" {{($customer->tax_type == 'NON VATABLE') ? 'selected' : ''}}>NON VATABLE</option>
          </select>
        </div>
      </div>
    </div>
        ...
        <div class="form-icon-user">
          <select class="select-line-of-business form-control custom-select" name="line_of_business">
            ...
            @foreach($line_of_business as $busines)
              <option value="{{$busines->id}}" {{($busines->id == $customer->line_of_business) ? 'selected' : ''}}>{{$busines->business_code.' '.$busines->business_title}}</option>
            @endforeach
          </select>
        </div>
      </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="custom-control custom-checkbox mt-4">
              @if($customer->type)
                <input class="custom-control-input" type="checkbox" value="Active" name="type[]" id="Active" {{str_contains($customer->type,'Active') ? 'checked' : ''}}>
              @else
          ...
          <div class="col-md-6">
            <div class="custom-control custom-checkbox mt-4">
              @if($customer->type)
                <input class="custom-control-input" type="checkbox" value="Withholding Agent" name="type[]" id="Withholding Agent" {{str_contains($customer->type,'Withholding Agent') ? 'checked' : ''}}>
              @else
