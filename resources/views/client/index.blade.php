...
<div class="all-button-box row d-flex justify-content-end">
  <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6" style="display: flex;margin-bottom: 10px">
    ...
    @can('create customer')
    <a href="#" data-size="2xl" data-url="{{ route('client.create') }}" data-ajax-popup="true" data-title="{{__('Create New Client')}}" class="btn btn-xs btn-white btn-icon-only width-auto commonModal">
      ...
      <div class="card-body py-0">
        <div class="table-responsive">
          <table class="table table-striped mb-0 dataTable">
            ...
            <tbody>
              @foreach ($clients as $k=>$customer)
              <tr class="cust_tr" id="cust_detail" data-url="{{route('client.show',$customer['id'])}}" data-id="{{$customer['id']}}">
                <td class="Id">
                  @can('show customer')
                  <a href="{{ route('client.show',$customer['id']) }}">
                    {{ AUth::user()->clientNumberFormat($customer['customer_id']) }}
                  </a>
                  @else
                ...
                <td class="Action">
                  <span>
                    ...
                    @else
                    @can('show customer')
                    <a href="{{ route('client.show',$customer['id']) }}" class="edit-icon bg-success" data-toggle="tooltip" data-original-title="{{__('View')}}">
                      <i class="fas fa-eye"></i>
                    </a>
                    @endcan
                    @can('edit customer')
                    <a href="#" class="edit-icon" data-size="2xl" data-url="{{ route('client.edit',$customer['id']) }}" data-ajax-popup="true" data-title="{{__('Edit Client')}}" data-toggle="tooltip" data-original-title="{{__('Edit')}}">
                    ...
                    {!! Form::open(['method' => 'DELETE', 'route' => ['client.destroy', $customer['id']],'id'=>'delete-form-'.$customer['id']]) !!}
                    {!! Form::close() !!}
                    @endcan
                    @endif
                  </span>
